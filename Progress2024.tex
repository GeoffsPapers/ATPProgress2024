...
% the fraction of solved problems, 
...
%  fives of which are in fixed coherent sets of problems.
% (i)~First ATP solutions of individual problems that are of direct interest to humans,
% (ii)~The fraction of ``never solved'' problems,
% (iii)~The fraction of solved problems,
% (iv)~The fraction of ``always easy'' problems,
% (v)~The decrease in problem difficulty ratings,
% (vi)~Shapley values over time and at fixed points in time.
...
problems.
% In order for such analyses to be meaningful 
...
%  e.g., Sledghammer \cite{PB10} for Isabelle \cite{NPW02}, 
...
\vspace*{-0.5em}
\paragraph{Solved:}
This measures the ratio of the number of problems solved to the most problems solved across all 
releases, wrt the least number solved across all releases.
%GEOFF TO CLARIFY THIS
...
progress.
% (in this case the temporal Shapley value is equal to the temporal marginal contribution).
Peaks identify releases where there was strong progress.
Following that, (non-temporal) Shapley analysis \cite{FK+16} is used to measure the contributions of
individual systems at each release.
Finally, temporal Shapley analysis for all systems and all releases is used to analyse the
contributions of the individual system versions when they were introduced, which reveals the 
numbers of problems they solved but that had never been solved before.
...
\item Time spent on machine learning based techniques, for 
      ...
      This work has focussed largely on large problem sets with fixed signatures and many
