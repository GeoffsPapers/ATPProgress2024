...
numbers symbols, the use of equality and arithmetic, etc.
...
increase with time, it is counterintuitive because the problem has not changed.
...
The basis for such sets are the SPCs (see Section~\ref{SPCs}), which provide a fine-grained 
partitioning of the TPTP problems so that each SPC is coherent.
Some SPCs that that capture compatible problem characteristics can be merged to form a 
{\em coherent SPC set}.
...
Figures~\ref{Plot_CNF_UEQ}-\ref{Plot_TH0_THM_NAR} plot the values for for each coherent SPC set
