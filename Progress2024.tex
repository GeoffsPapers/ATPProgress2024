...
The big picture is less bright, at least in terms problem difficulty ratings.
The reason are not definitely known, but we have thought of the following possible reasons:
\begin{itemize}
\item Systems gaining breadth of capability at the expense of depth, e.g., 
      E -- CNF and FOF in 2015, added TF0 in ???? \cite{}, TX0 and TH0 in 2018 \cite{SCV19};
      iProver -- CNF and FOF in 2015, added TF?(A) in ???? \cite{};
      Vampire -- CNF, FOF, TF0(A), in 2015, added THF, TXF, TF1 in ???? \cite{}.
\item New systems that take new approaches, solving different subsets of SPCs, e.g.,
      CSE\_E -- Added in 2019, combines the XXX calculus with E \cite{};
      Zipperposition -- Added in 2019, extends superposition to higher-order logic;
      Twee -- Added in 2018, solves CNF and FOF problems by transformation to UEQ;
\item Time spent on machine learning based techniques, for 
      ...
      and recently use of large language models to improve ATP performance \cite{}.
\item More focus on SMT
\item CASC causing incremental development, as discussed in \cite{SD24-CASC}.
\end{itemize}
